...
| 文件 | 操作 | 预估行数变化 |
|------|------|-------------|
| `skills/specpowers-review/SKILL.md` | 修改 | 791 → ~470（新增合理化表 ~40 + 保守精简 ~50 - 提取 ~230 到 refs/，净减 ~320） |
...
□ 本轮有退化场景？→ 有 → 退化声明含三要素且 status 非 complete？
```
...
> 审查流程中对这些协议的引用（"按退化声明标准协议输出"、"从 review-cache.json 读取"）在流程描述中保留上下文。
```
...
| 文件 | 行数 |
|------|------|
| `skills/specpowers-review/SKILL.md` | 791 → ~470 |
...
| 位置 | 改动 | ~节省 |
|------|------|------|
| 术语说明节 | "用户"定义删除；"主 Agent"/"微小任务"各压缩为一行 | 5 行 |
| 退化声明标准协议节导航提示 | "读者如需跟随线性审查流程，可跳过本节" — 删除 | 3 行 |
...
| specpowers-plan/apply 文件改动 | 本轮改动不涉及这两文件 | — |
