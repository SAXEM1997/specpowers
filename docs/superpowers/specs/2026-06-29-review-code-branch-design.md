...
| 10 | L577 Gate 触发协议汇总表 Gate 3 行 | 审查方式列同步更新 | `加强审查(≤2文件且≤200行) / UltraReview+对齐审查(其他)` |
| 11 | L392-399 分工对照表 | 审查对象列更新，Agent 数量 5→6 | UltraReview 列审查对象改为"大规模代码（其他情况）" |
...
| 6 | Gate 3 入口前（新增） | 新增步骤：执行 `git diff --stat <base>..HEAD` 计算 additions + deletions 作为修改总行数，传入审查决策 | — |
...
| # | 旧文本 | 新文本 |
|---|--------|--------|
| 1 | `≥10 文件` → UltraReview | `其他情况` → UltraReview + 对齐审查 |
| 2 | `<10 文件` → 加强审查 | `≤2 文件且 ≤200 行` → 加强审查 |
| 3 | 审查类型说明引用 `≥10 文件` 阈值 | 审查类型说明更新为级联条件阈值 |

### CLAUDE.md

| # | 旧文本 | 新文本 |
|---|--------|--------|
| 1 | `≥10 文件` 阈值引用 | `其他情况`（级联条件 2） |
| 2 | `<10 文件` 阈值引用 | `≤2 文件且 ≤200 行`（级联条件 1） |

### commands/specpowers.md

| # | 旧文本 | 新文本 |
|---|--------|--------|
| 1 | `≥10 文件` / `<10 文件` 阈值引用 | 替换为级联条件描述 |
...
2.5 **Why 边界跳变可接受**：条件 1 和条件 2 之间没有中间梯度。2文件201行或3文件80行的变更直接进入 6-agent 审查。理由：(a) 超过 200 行的单文件变更通常涉及重构级修改，单 Agent 不足以覆盖风险；(b) 超过 2 文件的变更需要跨文件协调，需多维度审查；(c) 增加中间梯度会增加路由复杂度，增量价值有限。
