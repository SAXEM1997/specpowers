...
| 条件 2 | 其他 | 其他 | UltraReview + 对齐审查 | 5-agent 团队审查 + 多模型渐进式中的对齐Agent（COVERED/MISSING/DRIFT 对照） |
...
| 文件 | 改动数 | 说明 |
|------|--------|------|
| `skills/specpowers-review/SKILL.md` | ~8处 | 主修改：决策树、流程图、Gate定义、章节条件、UltraReview对齐审查、分工对照表 |
| `skills/specpowers/SKILL.md` | 2处 | 路由表简化、微小任务审查列更新 |
| `skills/specpowers-apply/SKILL.md` | ~5处 | 前置检查、决策树摘要、验证0/1、code-review条件 |
...
| 3 | L77-78 流程图 Gate 3 | 更新审查方式标注 | `加强审查(≤2文件,≤200行) / UltraReview+对齐审查(其他)` |
...
└── 代码类（实现代码 / 项目结构 / 构建配置 / 项目创建或变更）
    ...
    │       （5-agent 团队审查 + 多模型渐进式中的对齐Agent COVERED/MISSING/DRIFT 对照）
    └── 用户可手动切换审查路径
```
...
- **加强审查（代码 ≤2 文件且 ≤200 行）时**：Gate 3 拆分为两部分：
  ...
  两者均通过方可进入 Phase 4。
- **UltraReview + 对齐审查（其他情况）时**：执行完整 Gate 3 UltraReview（5-agent 团队审查），并在审查维度中新增对齐审查 Agent，执行 COVERED/MISSING/DRIFT 对照（同多模型渐进式审查中的对齐 Agent 对照方法）。
```
...
| # | 位置 | 改动内容 |
|---|------|---------|
| 1 | L12 前置检查 item 4 | 整句替换为："如当前模式为微小任务，仍须加载 specpowers-review 执行 Gate 3 审查（走加强审查路径）。" |
| 2 | L43-46 Gate 3 决策树摘要 | 重写为 2 路径级联条件 |
...
3. **Why 对齐审查合并入 UltraReview 而非独立路径**：对齐审查（COVERED/MISSING/DRIFT）是 spec-compliance 的结构化检查，与 UltraReview 中 specs-reviewer 的合规检查互补——specs-reviewer 做定性审查，对齐 Agent 做结构化逐条对照。合并后 UltraReview 覆盖 6 个维度。
4. **微小任务执行模式保留**：入口技能的微小任务定义（1-3文件, <50行）控制 Phase 0-2 是否跳过，与审查阈值独立。微小任务 Phase 3 完成后仍走加强审查。
...
- 旧阈值（`≥10文件`、`<10文件`、`微小任务跳过`）全部替换，不留残留引用
