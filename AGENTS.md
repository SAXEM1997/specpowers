...
2. **Phase 0 独立于 OpenSpec**: Phase 0 使用 `brainstorming` 完成需求澄清+方案设计，产物为 `docs/superpowers/specs/<name>-design.md`。Phase 1 仅做格式转换+强制对照验证，不重复做需求分析。
...
7. **审查防退化机制**: specpowers-review 内置三层防退化防御——审查纪律自检（合理化表 + Red Flags）、多轮审查防偷懒协议（主 Agent 自检）、审查层级边界（两层边界原则——分析不受限，评论受限制）。收敛判定为默认继续制（`[CONVERGENCE_CHECK]` 强制标记 + 5 触发条件【p0_raw>1 / p1_raw>5 / P0-P2 合计>10 / 全级>20 / p1_raw 轮间新增>3，全部基于原始发现数 p*_raw】+ review 内部循环 + 用户干预窗口；action=exit 即最终通读，无额外数值条件）。合并验证按问题数 N（raw_count_sum 口径）动态部署：p0_raw≥3 或 N>15→独立监督 Agent 完整 4 维；N∈[1,15] 且 p0_raw≤2→Step 5 兼并（p0_raw≥1 追加严重度校准抽查，构成 P0 重审载体）；N==0→trivial。修复阶段默认全量修复 P0-P3（子Agent逐条分析），同文件不并发硬约束防止编辑冲突。
8. **状态机主导 + hooks 默认 off**: 入口启动协议融合 Decision Core——`node <SKILL_BASE>/scripts/workflow-state.mjs status`（`<SKILL_BASE>` 见入口技能「脚本路径解析」节）状态机先行判定 Phase，`.gate-passed-<N>` 文件（name 绑定）最终裁决，Phase 自动检测降级为回退路径；4 个子技能（design/plan/apply/archive）Gate 出口调 `workflow-guard.mjs exit <phase> --apply`（token 先写、guard 后调，子技能为唯一责任方）；hooks 默认不注册（模板在 scripts/，启用需显式复制到 .claude/settings.json）。微小任务豁免（不 init 状态机，保留轻量行为）。
