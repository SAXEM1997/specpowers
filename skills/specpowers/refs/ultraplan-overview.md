...
- 需要 MainAgent 拥有**尽可能大的上下文窗口**——按能力等级优先选择当前环境可用的、窗口最大的模型（示例：1M 级上下文的强推理模型；具体品牌随各环境可用模型变化，不是规定）
- 如果你的模型上下文窗口偏小（示例：200K 级），**务必在执行前先清空上下文**（Claude Code：`/clear`；其他平台用等价的清空或新会话手段）
- 窗口不足时才做上下文压缩，效果通常很差——要在执行前就留足空间
- 保持充足的上下文空间是 UltraPlan 成功执行的关键前提

如果对本地化的 UltraPlan 有任何疑问或建议，欢迎在本仓库提 Issues 交流。
