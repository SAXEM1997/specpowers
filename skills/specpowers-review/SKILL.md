...
> **规模分桶度量（按对象类型区分）**：代码类按 `file_count`（变更文件数）——微小 1-3 / 中等 4-19 / 复杂 20-49 / 大规模 50+。文档类按 `line_count`（文档总行数）——微小 ≤100 / 中等 101-300 / 复杂 301-600 / 大规模 600+。文档类的 file_count 无区分度（Gate 0/2 始终 1 文件，Gate 1 通常 4-6 文件），改用 line_count 度量内容复杂度。详见协议 6 `bucket_doc` 函数。
